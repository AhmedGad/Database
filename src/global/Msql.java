...
public class Msql implements MiniSqlTreeConstants {
	...
	protected static String PATH = System.getProperty("user.name")
			+ ".minibase";
	...
	public static void main(String[] args) {
		...
		while (true) {
			System.out.println("COUNT IS + " + FileScan.count);
			...
			if (node.isHelp) {
				System.out
						.println("HACK: just enter ';' to see available commands.");
				continue;
			}
			if (node.isStats) {
				...
				reads = Minibase.DiskManager.getReadCount(); // ignore
																// getAllocCount
				writes = writes2;
				allocs = allocs2;
				continue;
